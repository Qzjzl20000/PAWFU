...
class MatchingAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if att_type == 'general2':
            ...
            self.transform = nn.Linear(cand_dim + mem_dim,
                                       alpha_dim,
                                       bias=False)
    ...
    def forward(self, M, x, mask=None):
        ...
        if self.att_type == 'dot':
            ...
            if type(mask) == type(None):
                mask = torch.ones(M.size(1),
                                  M.size(0)).type(M.type()).to(self.device)
            M_ = M.permute(1, 2, 0)  # batch, mem_dim, seqlen
            x_ = self.transform(x).unsqueeze(1)  # batch, 1, mem_dim
            alpha_ = F.softmax((torch.bmm(x_, M_)) * mask.unsqueeze(1),
                               dim=2)  # batch, 1, seqlen
...
class DialogueRNNCell(nn.Module):
    ...
    def __init__(self,
        ...
        if context_attention == 'simple':
            ...
            self.attention = MatchingAttention(D_g, D_m, D_a,
                                               context_attention, device)
    ...
    def forward(self, U, qmask, g_hist, q0, e0):
        ...
        g_ = self.g_cell(
            ...
            torch.zeros(U.size()[0], self.D_g).type(U.type()).to(self.device)
            if g_hist.size()[0] == 0 else g_hist[-1])
        g_ = self.dropout(g_)
        if g_hist.size()[0] == 0:
            c_ = torch.zeros(U.size()[0],
                             self.D_g).type(U.type()).to(self.device)
            alpha = None
        else:
...
class DialogueRNN(nn.Module):
    ...
    def forward(self, U, qmask):
        ...
        g_hist = torch.zeros(0).type(U.type()).to(
            self.device)  # 0-dimensional tensor
        q_ = torch.zeros(qmask.size()[1],
                         qmask.size()[2], self.D_p).type(U.type()).to(
                             self.device)  # batch, party, D_p
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.matchatt = MatchingAttention(D_e,
                                          D_e,
                                          att_type='general2',
                                          device=0)
    ...
    def forward(self, U, qmask, umask=None, att2=False):
        ...
        if att2:
            ...
            for t in emotions:
                att_emotions.append(
                    self.matchatt(emotions, t, mask=umask)[0].unsqueeze(0))
...
class BiE2EModel(nn.Module):
    ...
    def __init__(self,
        ...
        #self.linear3     = nn.Linear(D_h, D_h)
        self.smax_fc = nn.Linear(D_h, n_classes)
        self.embedding = nn.Embedding(word_embeddings.shape[0],
    ...
    def forward(self, data, att2=False):
        ...
        emotions_f, alpha_f = self.dialog_rnn_f(U,
                                                qmask)  # seq_len, batch, D_e
...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, output_size, filters,
    ...
    def forward(self, x, umask):
        ...
        emb = self.embedding(
            x
        )  # (num_utt * batch, num_words) -> (num_utt * batch, num_words, 300)
        ...
        features = F.relu(self.fc(self.dropout(
            concated)))  # (num_utt * batch, 150) -> (num_utt * batch, 100)
        features = features.view(
            num_utt, batch,
            -1)  # (num_utt * batch, 100) -> (num_utt, batch, 100)
        mask = umask.unsqueeze(-1).type(
            FloatTensor)  # (batch, num_utt) -> (batch, num_utt, 1)
        mask = mask.transpose(0,
                              1)  # (batch, num_utt, 1) -> (num_utt, batch, 1)
        mask = mask.repeat(
            1, 1,
            self.feature_dim)  #  (num_utt, batch, 1) -> (num_utt, batch, 100)
        features = (features * mask
                    )  # (num_utt, batch, 100) -> (num_utt, batch, 100)
...
class DailyDialogueModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.cnn_feat_extractor = CNNFeatureExtractor(
            vocab_size, embedding_dim, cnn_output_size, cnn_filters,
    ...
    def forward(self, input_seq, qmask, umask):
        ...
        # hidden = F.relu(self.linear(emotions))
        hidden = self.dropout(hidden)
        log_prob = F.log_softmax(self.smax_fc(hidden),
                                 2)  # seq_len, batch, n_classes
